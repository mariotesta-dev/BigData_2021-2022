...
public class DriverBigData extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		*/
		Path inputPath;
		Path outputDir;
		int numberOfReducersJob;

		// Parse the parameters for the set up of the first job
		inputPath = new Path(args[0]); //That should be "PatchedServers.txt"
		outputDir = new Path(args[1]);
		numberOfReducersJob = Integer.parseInt(args[2]);
		...
		job.setNumReduceTasks(numberOfReducersJob); // >1
